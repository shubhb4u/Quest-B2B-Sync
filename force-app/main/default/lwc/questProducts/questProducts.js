...
export default class QuestProducts extends NavigationMixin(LightningElement) {
    ...
    fetchProducts() {
        this.isLoading = true;
        this.error = null;
        console.log('Fetching product with search key:' , this.searchKey);
        getProductRecs({searchKey: this.searchKey})
            .then((data) => {
    ...
    fetchProductsByCategory() {
        getProdId({ catId: this.categoryId, searchKey: this.searchKey  })
            .then((data) => {
                const newProducts = data.map(product => {
